...
function SellRequestPage() {
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      setSellRequests(prev => prev.map(r => (r._id === editingId ? { ...r, ...data.request } : r)));
      cancelEdit();
    } catch (err) {
  ...
  return (
    <div className="user-my-sellrequests">
      {sellRequests.length === 0 ? (
        <p className="user-my-sellrequests__empty">
          You haven&apos;t created any sell requests yet.
        </p>
      ) : (
              ...
              {!isEditing ? (
                <>
                  ...
                  {req.location?.coordinates?.length === 2 && (
                    <p>
                      <strong>Location:</strong> {req.location.coordinates[1]},{' '}
