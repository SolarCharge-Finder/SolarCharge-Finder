...
describe('StationDetails', () => {
  ...
  it('alerts when trying to submit review while not logged in', async () => {
    axios.get.mockImplementation((url) => {
      if (url.includes('/api/stations/')) return Promise.resolve({ data: { data: { _id: '1', name: 'S1', photos: [], location: { coordinates: [0,0] }, connectors: [] } } })
  ...
  it('submits review when logged in', async () => {
    ...
    axios.get.mockImplementation((url) => {
      if (url.includes('/api/stations/')) return Promise.resolve({ data: { data: { _id: '1', name: 'S1', photos: [], location: { coordinates: [0,0] }, connectors: [] , rating:0 } } })
    ...
    expect(axios.post).toHaveBeenCalledWith('/api/reviews', expect.objectContaining({ stationId: '1', rating: expect.any(Number) , comment: 'Nice place' }), expect.any(Object))
  })
})
