...
const normalizeReviews = (list = []) =>
  list.map(review => ({
    ...
    userName: review.userName ?? review.user?.name ?? review.user?.email ?? 'Anonymous user',
  }));
  ...
  const renderStars = (rating = 0) => {
    return Array.from({ length: 5 }, (_, index) => (
      <span
        key={index}
        className={
          index < rating ? 'review-card__star' : 'review-card__star review-card__star--muted'
        }
      >
        ★
      </span>
          ...
          <div className="admin-reviews-by-station">
            {(() => {
              const byStation = reviews.reduce((acc, review) => {
                const sid = String(
                  review.station?._id ?? review.station?.id ?? review.station ?? 'unknown'
                );
              ...
              if (!entries.length)
                return <div className="admin-empty-state">No reviews available.</div>;
                ...
                return (
                  <div key={stationId} className="admin-station-reviews-block">
                    ...
                    >
                      ...
                      <span className="admin-station-reviews-block__count">
                        ...
                        {stationReviews.map(review => {
                          ...
                          return (
                            <article key={reviewId} className="review-card">
                              ...
                              {review.comment && (
                                <p style={{ marginTop: '0.75rem', color: 'var(--admin-muted)' }}>
                                  {review.comment}
                                </p>
                              )}
