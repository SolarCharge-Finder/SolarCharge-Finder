...
function StationCard({ station, onEdit, onDelete, onCardClick }) {
  ...
  // Format location
  const location = [station.city, station.district].filter(Boolean).join(', ').toUpperCase();
  ...
  return (
    ...
    >
      ...
      <div className="station-card-content">
        ...
        {location && <p className="station-location">{location}</p>}
...
function ManageStations() {
  ...
  const handleEdit = stationId => {
    ...
    setFormData({
      ...
      connectors:
        stationToEdit.connectors && stationToEdit.connectors.length > 0
