...
vi.mock('../../../client/src/context/useAuth', () => ({
...
describe('AuthPage', () => {
  ...
  it('shows success message on signup', async () => {
    ...
    const signUpButton = screen.getAllByText(/Sign Up/i).find(el => el.className && el.className.includes('primary-btn'))
    // Fill signup inputs
  ...
  it('calls login on successful signin', async () => {
    ...
    global.fetch.mockResolvedValueOnce({ ok: true, json: async () => ({ data: { token: fakeToken, user: fakeUser } }) })
