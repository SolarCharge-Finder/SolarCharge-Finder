...
describe('Review rating math', () => {
  ...
  test('addReview updates station rating and totalRatings', async () => {
    ...
    mockCreateReview.mockResolvedValue({ _id: 'r1', rating: 5, user: 'u1', populate: jest.fn().mockResolvedValue({}) })
  ...
  test('updateReview recalculates rating when rating changed', async () => {
    ...
    const review = { _id: 'r1', rating: 4, station: 's1', user: 'u1', save: jest.fn(), populate: jest.fn().mockResolvedValue({}) }
